...
bool CONTRAT_CLIENT1::modifier()
{
        ...
        query.prepare("UPDATE CONTRAT_CLIENT1 SET date_sign=:date_signn,date_expir=:date_expirr,nom_societe=:nom_societee,duree_contrat=:duree_contratt,domaine=:domainee WHERE num_contrat=:num_contrat" );

        query.bindValue(":num_contratt",num );

        query.bindValue(":date_signn",date_sign );
        query.bindValue(":date_expirr",date_expir );
        query.bindValue(":nom_societee",nom_societe);
        query.bindValue(":duree_contratt",duree_contrat );
        query.bindValue(":domainee",domaine);
        return query.exec();
    }
